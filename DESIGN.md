...
- **Mark hierarchy:** the **cream terracotta tile is the lead mark and is applied by default wherever the brand appears — including in-product nav, headers, and footers, not only brand/identity slots.** It renders in two forms depending on space: the **full logo with wordmark** (`assets/logos/orbti-logo.png`) for brand blocks where the tile has room (heroes, footer brand columns, splash, og:image) and the baked `ORBTI` lettering can read or is carried by adjacent copy; and the **wordmark-less icon** (`build/icon.png` / `assets/logos/orbti-icon.png`) for compact or square slots — **nav bars, app headers**, favicon, app icon, avatar, social profile. In a nav/header, lead with the **icon tile paired with an external flat `ORBTI` wordmark** (`assets/logos/orbti-wordmark-*.svg`, dark ink on light surfaces, light ink on dark) at ~32px tile + ~18px wordmark — the icon tile carries the identity and the separate wordmark keeps the name crisp at small sizes, where the full tile's baked lettering would empaste. The flat **`ORBTI` wordmark on its own** is now only a **constrained fallback** — inline text contexts, monochrome/one-color reproductions, or slots too tight for any tile — never the default brand gesture. Use the **reverse coral tile** only as an occasional, intentional dark-surface variant (see above). Use the **product lockups** (Core / Pulse / Horizon) only for those sub-brands.
