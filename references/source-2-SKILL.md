...
- **`assets/`** — real preserved brand marks: `logos/` (wordmarks + Core/Pulse/Horizon lockups), `icons/` (orbital product icons, light + dark), `backgrounds/` (hero PNGs).
...
> Note: there is **no `build/`** directory (the evidence is a static site with no
> runtime/installer icons) and **no `fonts/`** directory (fonts load from Google
> Fonts CDN, bound in `colors_and_type.css`).
